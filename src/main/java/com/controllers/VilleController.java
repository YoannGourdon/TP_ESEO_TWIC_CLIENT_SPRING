...
public class VilleController {
	...
	public String getCalculDistance(@RequestParam("ville1") String ville1,
		...
		try {
			...
			double latitudeVille1 = Float.parseFloat(ville1.split(";")[0]) * Math.PI/180;
			double longitudeVille1 = Float.parseFloat(ville1.split(";")[1]) * Math.PI/180;
			double latitudeVille2 = Float.parseFloat(ville2.split(";")[0]) * Math.PI/180;
			double longitudeVille2 = Float.parseFloat(ville2.split(";")[1]) * Math.PI/180;
			
			int rayonTerreKm = 6371;
			
			//Formule de Haversine
			//Source du calcul: https://fr.wikipedia.org/wiki/Formule_de_haversine
			double sinCarreLat = Math.pow(Math.sin((latitudeVille2 - latitudeVille1)/2), 2);
			double sinCarreLong = Math.pow(Math.sin((longitudeVille2 - longitudeVille1)/2), 2);
			
			
			double calculKm = 2 * rayonTerreKm * Math.asin(Math.sqrt(sinCarreLat + (Math.cos(latitudeVille1) * Math.cos(latitudeVille2) * sinCarreLong)));
					
					
			model.addAttribute("calcul", calculKm);
			String villes = restTemplate.getForObject(URL, String.class);
			model.addAttribute("villes", getListVillesFromString(villes));	
		} catch (NullPointerException e) {}
		return "calculDistance";
